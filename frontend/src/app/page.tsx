...
async function fetchRooms(): Promise<Room[]> {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_API_URL || "/api"}/roulette/sessions`
  );
...
export default function Home() {
  ...
  const handleCreateRoomSubmit = async (roomKey: string, password?: string) => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "/api"}/rooms/auth`,
        {
          method: "POST",
      ...
      if (!res.ok) {
        throw new Error(
          (await res.json()).error || "Не удалось создать комнату"
        );
      }
      ...
      enqueueSnackbar(`Комната "${roomKey}" успешно создана!`, {
        variant: "success",
      });
    } catch (error: unknown) {
      enqueueSnackbar(
      ...
      <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
        {rooms && rooms.length > 0
          ? rooms.map((room) => (
              <Box
                key={room.key}
                sx={{ backgroundColor: "#2a2a2a", padding: 2, borderRadius: 1 }}
              >
                ...
                {room.historyLength !== undefined && (
                  <Typography variant="body2" color="text.secondary">
                    История: {room.historyLength} чисел
                  </Typography>
                )}
                <Button
                  variant="contained"
